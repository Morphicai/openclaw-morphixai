...
description: "[已废弃] 请使用 office-link skill。Pipedream 代理功能已整合到 MorphixAI (office_link) 中。"
metadata:
  openclaw:
    emoji: "🔐"
    deprecated: true
...
| Pipedream Connect API Proxy | `office_link: action: proxy` |
| Pipedream Connected Accounts | `office_link: action: list_accounts` |
| 手动构建 proxy 请求 | 使用 `office_*` 专用工具（自动处理 URL/认证/格式） |

## 为什么迁移

- **统一入口**：只需 1 个 API Key，不再需要 Pipedream Token + Project ID
- **专用工具**：GitLab/GitHub/Jira 等有 `office_*` 封装，无需手写 API URL
