...
通过 `office_outlook` 工具管理 Microsoft Outlook 邮箱：读取、搜索、发送和回复邮件。

## 前置条件

1. 配置 `MORPHIXAI_API_KEY` 环境变量
2. 用户需要通过 `office_link` 工具链接 Outlook 账号（app: `microsoft_outlook`）
...
```
1. office_outlook: list_messages, top: 10
     → 检查未读邮件（isRead: false）
2. office_outlook: get_message, message_id: "xxx"
     → 查看具体内容
3. office_outlook: reply_to_message, message_id: "xxx", comment: "回复内容"
```

### 搜索特定邮件

```
1. office_outlook: search_messages, query: "from:boss@company.com 本周"
```
